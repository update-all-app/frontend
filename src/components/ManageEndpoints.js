...
export default function ManageEndpoints({ business }) {
  ...
  const otherBusinesses = state.data.businesses.filter(
    (b) => b.id !== business.id
  );
  const [loading, setLoading] = useState(false);
  const [loadingButtons, setLoadingButtons] = useState(
    getAllButtonIdentifiersForState()
  );
  ...
  function getAllButtonIdentifiersForState() {
    return SUPPORTED_SERVICES.reduce((outerAccum, ss) => {
      ...
      const pagesAuthorized = linkedServices.reduce(
        (serviceAccum, linkedService) => {
  ...
  async function handleConnect({ serviceName, pageId, providerOauthTokenId }) {
  ...
  async function handleDisconnect({ locationServiceId, serviceName, pageId }) {
    const buttonId = `${serviceName}-${pageId}`;
    setLoadingButtons((loadingButtons) => {
      const nextState = { ...loadingButtons };
      nextState[buttonId] = true;
  ...
  function isPageConnectedToAnotherBusiness(providerOauthTokenId, pageId) {
    for (let otherBusiness of otherBusinesses) {
      if (
        isPageConnectedToBusiness(providerOauthTokenId, pageId, otherBusiness)
      ) {
        return true;
      }
    }
  ...
  function isPageConnectedToBusiness(providerOauthTokenId, pageId, business) {
    return !!business.connectedPages.find((page) => {
      return (
        page.providerOauthTokenId === providerOauthTokenId &&
        page.pageId === pageId
      );
    });
  }
  ...
  const renderLinkedServices = (service) => {
    ...
    if (linkedServices.length > 0) {
      const pages = linkedServices.flatMap((s) =>
        s.pageData.map((pageData) => ({
          ...pageData,
          providerOauthTokenId: s.providerOauthTokenId
        }))
      );
      return (
        <Accordion headerText={service.name} key={hash(service.value)}>
          <div>
            {pages.map((pageData) => {
              const pageIsConnected = isPageConnectedToBusiness(
                pageData.providerOauthTokenId,
                pageData.id,
                business
              );
              const pageIsConnectedToDifferentBusiness = isPageConnectedToAnotherBusiness(
                pageData.providerOauthTokenId,
                pageData.id
              );
              ...
              if (pageIsConnected) {
                ...
                onClick = () =>
                  handleDisconnect({
                    locationServiceId: getLocationServiceId(
                      pageData.providerOauthTokenId,
                      pageData.id
                    ),
                    serviceName: service.value,
              ...
              return (
                <Card clickable={false} classNames={'p-4'}>
                  ...
                  <div className='flex flex-col justify-between items-center height-full'>
                    ...
                    <LoadingButton
                      value={value}
                      loading={
                        loadingButtons[`${service.value}-${pageData.id}`]
                      }
      ...
      return (
        <Accordion headerText={service.name} key={hash(service.value)}>
          <p className='p-4'>
            You have not authorized {service.name}.{' '}
            <button
              className='focus:outline-none text-blue-600'
              onClick={() => history.push('/authorize-services')}
            >
              Do it here
            </button>
          </p>
