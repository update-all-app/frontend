...
export default function AuthorizeServices(props) {
  ...
  const syncFacebook = async () => {
    ...
    try {
      ...
      const { provider, provider_uid, id, label, page_data } = res;
      const service = {
        provider,
        userID: provider_uid,
        label,
  ...
  const syncGoogle = async () => {
    ...
    try {
      ...
      const backendRes = await GoogleApiManager.createGoogleConnection(
        res.code
      );
      const { provider, provider_uid, id, label, page_data } = backendRes;
      const service = {
        provider,
        userID: provider_uid,
        label,
      ...
      <div className='m-12'>
        <Card clickable={false}>
          ...
          <div className='flex flex-col justify-between items-center'>
            <div className='flex flex-col'>{renderServicesFor('facebook')}</div>
            <LoadingButton
        ...
        <Card clickable={false}>
          ...
          <div className='flex flex-column justify-center items-center'>
            <div>{renderServicesFor('instagram')}</div>
            <LoadingButton
        ...
        <Card clickable={false}>
          ...
          <div className='flex flex-column justify-center items-center'>
            <div>{renderServicesFor('twitter')}</div>
            <LoadingButton
        ...
        <Card clickable={false}>
          ...
          <div className='flex flex-column justify-center items-center'>
            <div>{renderServicesFor('google')}</div>
            <LoadingButton
