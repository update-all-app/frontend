...
export default function EditIrregularSchedule(props) {
  ...
  const onSave = async (event) => {
    ...
    if (isValid) {
      ...
      if (isNewEvent) {
        ...
        try {
          const res = await BusinessApiManager.createIrregularEventForLocation(
        ...
        try {
          const res = await BusinessApiManager.updateIrregularEvent(
            optimisticEvent
          );
  ...
  const validateEvent = (event) => {
    ...
    if (event.end < rightNow) {
      ...
      event.start < rightNow &&
      !datesInSameDay(event.start, rightNow)
    ) {
      message = 'Events must be in the future';
      return [false, message];
    }
  ...
  const renderErrorBanner = () => {
    return error ? (
      <ErrorBanner message={error} onExit={() => setError(null)} />
    ) : null;
  };
