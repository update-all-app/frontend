...
export default function ViewBusinessCalendar(props) {
  ...
  const getScheduleMemo = useCallback(
    ...
    [props.business.locationIds]
  );

  useEffect(() => {
    const getSchedule = async (startDate, endDate) => {
      setLoading(true);
      const res = await getScheduleMemo(startDate, endDate);
      setEvents(res);
      setLoading(false);
    };
  ...
  useEffect(() => {
    ...
    if (dateRangeStart) {
      getSchedule(
        dateToHTMLString(dateRangeStart),
        dateToHTMLString(dateRangeEnd)
      );
    }
  ...
  const renderDateRange = () => {
    return (
      <>
        <span className='font-bold'>{formatDateShort(dateRangeStart)}</span> to{" "}
        <span className='font-bold'>{formatDateShort(dateRangeEnd)}</span>
      </>
    );
  };
  ...
  const displayEvents = (dayEvents) => {
    return dayEvents.map((e) => (
      <div className='mb-2'>
        <p className='p-1 bg-gray-100 inline-block w-1/2'>{`${time24To12(
          e.start_time
        )} - ${time24To12(e.end_time)}`}</p>
      </div>
    ));
  };
  ...
  const renderDay = (dayNumber) => {
    ...
    const currentDate = datePlusDays(
      dateRangeStart,
      getDaysFromStartFromDayOfWeek(dayNumber)
    );
  ...
  return loading ? (
    ...
    <div className='m-8'>
      <div className='border-b-2 p-2'>
        ...
        <h1 className='pl-4 pt-4 align-bottom leading-3 inline-block text-3xl mb-0 mt-4 secdark p-2'>
          {dateRangeStart.toLocaleString("default", { month: "long" })},{" "}
