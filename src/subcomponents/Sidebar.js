...
export default function Sidebar(props) {
  ...
  const renderLinks = () => {
    return links.map((link, i) => {
      ...
      let icon =
        props.children && props.children.length > i ? props.children[i] : null;

      let callback =
        callbacks && callbacks.length > i ? callbacks[i] : () => {};
      ...
      return (
        <li key={hash(link)} className='mr-3 flex-1 mb-5 block h-full'>
          <button
            onClick={callback}
            className={`ml-2 ${mtClass} block ${activatedClass} py-2 pl-4 w-full text-left text-gray-500 no-underline border-l border-secdark sidebar-link cursor-pointer focus:outline-none`}
          >
            {icon}
            <span className='pb-1 inline text-xs hidden md:inline lg:text-base'>
              {link}
            </span>
          </button>
        </li>
